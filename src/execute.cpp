...
uint32_t Core::branch_unit(const Instr &instr, uint32_t rs1_data, uint32_t rs2_data, uint32_t rd_data, uint32_t PC) {
  ...
  case BrOp::BLTU: {
    br_taken = ((uint32_t)rs1_data<(uint32_t)rs2_data)?true:false;// TODO:
    break;
  }
  case BrOp::BGEU: {
    br_taken = ((uint32_t)rs1_data>=(uint32_t)rs2_data)?true:false;// TODO:
  ...
  if (br_op != BrOp::NONE) {
    ...
    if (br_taken) {
      ...
      if (br_op != BrOp::JAL && br_target != next_PC) { //If JALR or branch instructions, CPU might predict in advance if a branch operation is taken or not. 
        //If prediction is correct we proceed, if not, we flush following not taken instructions and start again with new branched instructions.
        PC_ = br_target;// TODO:
        // flush pipeline
        if_id_.reset();
