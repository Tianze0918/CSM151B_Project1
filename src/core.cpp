...
void Core::id_stage() {
  ...
  // check data hazards, if hazard happens, we wait till next cycle?
...
bool Core::check_data_hazards(const Instr &instr) {
  ...
  //ex_mem_ is a pipeline register template class object of type ex_memt, ex_mem.empty() returns false when em_mem's push method is not called
  if (!ex_mem_.empty()) { //This loop is checking if pipeline register between execution and data memory access have been pushed
    //When does ex_mem.push() happens? It happens at ex_stage, but check_data_hazard is called in id stage. So we are trying to see if ex_stage 
    //is called before id_stage. But how does the stages synchronize? Through tick which is called for every cycle. 
